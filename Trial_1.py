...
# #                 )
# #                 st.success("Questionnaire added successfully.")
# #                 st.cache_data.clear()
...
# #             key="dma_industry_filter",
# #         )
# #     with d2:
# #         selected_context_types = st.multiselect(
# #             "Filter Context Type",
# #             options=sorted(
# #                 [v for v in questionnaire_df[context_type_col].dropna().astype(str).unique() if v.strip()]
# #             ),
# #             placeholder="All Context Types",
# #             key="dma_context_type_filter",
# #         )
...
# #         filtered_df = filtered_df[filtered_df[context_type_col].astype(str).isin(selected_context_types)]

# #     filtered_df = filtered_df.reset_index().rename(columns={"index": "_row_id"})

# #     st.caption("Use each column header menu (⋮) for Sort / Filter, just like Excel.")

# #     dma_gb = GridOptionsBuilder.from_dataframe(filtered_df)
# #     dma_gb.configure_default_column(
# #         editable=False, sortable=True, filter=True, floatingFilter=True, resizable=True
# #     )
...
# #         filterParams={"buttons": ["reset", "apply"], "excelMode": "windows"},
# #     )
# #     dma_gb.configure_column(question_col, editable=True)
# #     dma_gb.configure_column(mandarin_col, editable=True)
# #     dma_gb.configure_column(updated_on_col, editable=False)
# #     dma_gb.configure_grid_options(stopEditingWhenCellsLoseFocus=True)
...
# #     if st.button("💾 SAVE ALL CHANGES", type="primary", key="dma_save_all_changes"):
# #         try:
# #             latest_df, latest_sheet = read_latest_dma_questionnaire()
...
# #                 st.success("IRO entry added successfully.")
# #                 st.cache_data.clear()
...
# #     prominent_cols = st.columns(3)
# #     with prominent_cols[0]:
# #         selected_iro_types = st.multiselect(
# #             "Filter IRO Type",
# #             options=["Impact", "Risk", "Opportunity"],
# #             placeholder="All IRO Types",
# #             key="iro_type_filter",
# #         )
# #     with prominent_cols[1]:
# #         selected_industries = st.multiselect(
# #             "Filter Industry",
# #             options=sorted([v for v in working_iro_df[industry_col].dropna().astype(str).unique() if v.strip()]),
# #             placeholder="All Industries",
# #             key="iro_industry_filter",
# #         )
# #     with prominent_cols[2]:
# #         selected_time_horizons = st.multiselect(
# #             "Filter Time Horizon",
# #             options=sorted([v for v in working_iro_df["_Time Horizon"].dropna().astype(str).unique() if v.strip()]),
# #             placeholder="All Time Horizons",
# #             key="iro_time_horizon_filter",
# #         )

# #     additional_filter_cols = st.columns(3)
# #     with additional_filter_cols[0]:
# #         type_of_impact_filter = st.multiselect(
# #             "Filter Type of Impact",
# #             options=sorted(
# #                 [v for v in working_iro_df[type_of_impact_col].dropna().astype(str).unique() if v.strip()]
# #             )
# #             if type_of_impact_col
# #             else [],
# #             placeholder="All Type of Impact",
# #             key="iro_type_of_impact_filter",
# #         )
# #     with additional_filter_cols[1]:
# #         nature_of_impact_filter = st.multiselect(
# #             "Filter Nature of Impact",
# #             options=sorted(
# #                 [v for v in working_iro_df[nature_of_impact_col].dropna().astype(str).unique() if v.strip()]
# #             )
# #             if nature_of_impact_col
# #             else [],
# #             placeholder="All Nature of Impact",
# #             key="iro_nature_of_impact_filter",
# #         )
# #     with additional_filter_cols[2]:
# #         selected_country = st.selectbox(
# #             "Filter Country",
# #             options=["All"] + country_filter_options,
# #             key="iro_country_filter",
# #         )
...
# #         ]
# #     if selected_iro_types:
# #         filtered_iro_df = filtered_iro_df[filtered_iro_df["_IRO Type"].astype(str).isin(selected_iro_types)]
# #     if selected_industries:
# #         filtered_iro_df = filtered_iro_df[filtered_iro_df[industry_col].astype(str).isin(selected_industries)]
# #     if selected_time_horizons:
# #         filtered_iro_df = filtered_iro_df[filtered_iro_df["_Time Horizon"].astype(str).isin(selected_time_horizons)]
# #     if type_of_impact_col and type_of_impact_filter:
# #         filtered_iro_df = filtered_iro_df[
# #             filtered_iro_df[type_of_impact_col].astype(str).isin(type_of_impact_filter)
# #         ]
# #     if nature_of_impact_col and nature_of_impact_filter:
# #         filtered_iro_df = filtered_iro_df[
# #             filtered_iro_df[nature_of_impact_col].astype(str).isin(nature_of_impact_filter)
# #         ]
# #     if selected_country != "All":
# #         matching_country_cols = [
# #             col for col in filtered_iro_df.columns if str(col).split(".")[0] == selected_country
# #         ]
# #         if matching_country_cols:
# #             country_mask = filtered_iro_df[matching_country_cols].applymap(_has_meaningful_geo_value).any(axis=1)
# #             filtered_iro_df = filtered_iro_df[country_mask]
...
# #         filterParams={"buttons": ["reset", "apply"], "excelMode": "windows"},
# #     )
# #     iro_gb.configure_column("Description", editable=True)
# #     iro_gb.configure_grid_options(stopEditingWhenCellsLoseFocus=True)
...
# #     if st.button("💾 SAVE ALL CHANGES", type="primary", key="iro_save_all_changes"):
# #         try:
# #             latest_df, latest_sheet = read_latest_iro_data()
...
# #         "KPI Type",
# #         options=["Standard KPI", "Tabular — Static", "Tabular — Dynamic"],
# #         index=0,
...
# #         table_name = in_title
# #         in_desc = st.text_area("Framework Description")
# #         in_ref_code = st.text_input("Framework Reference Code")
...
# #             @st.dialog(f"Edit Cell Details — Row {i}, Column {j}")
# #             def edit_cell_dialog(row_i, col_j):
# #                 existing = st.session_state.tabular_cell_data.get((row_i, col_j), {})
# #                 ctype = st.selectbox(
# #                     "KPI Type",
# #                     options=["Numeric (Table)", "TextArea (Narrative in Table)"],
# #                     index=0 if existing.get("kpi_type", "Numeric (Table)") == "Numeric (Table)" else 1,
# #                     key=f"dlg_type_{row_i}_{col_j}",
# #                 )
# #                 ctitle = st.text_input(
# #                     "Title",
# #                     value=existing.get("title", f"Cell R{row_i}C{col_j}"),
# #                     key=f"dlg_title_{row_i}_{col_j}",
# #                 )
# #                 cdesc = st.text_area(
# #                     "Description",
# #                     value=existing.get("desc", in_desc),
# #                     key=f"dlg_desc_{row_i}_{col_j}",
# #                 )
# #                 cagg = st.selectbox(
# #                     "Aggregation",
...
# #                     else 0,
# #                     key=f"dlg_agg_{row_i}_{col_j}",
# #                 )
# #                 cformula = st.text_input(
# #                     "Cell Formula",
# #                     value=existing.get("formula", ""),
# #                     key=f"dlg_formula_{row_i}_{col_j}",
# #                 )
# #                 if st.button("Save Cell", key=f"dlg_save_{row_i}_{col_j}"):
# #                     st.session_state.tabular_cell_data[(row_i, col_j)] = {
# #                         "kpi_type": ctype,
# #                         "title": ctitle,
# #                         "desc": cdesc,
# #                         "aggregation": cagg,
...
# #                         cell_payload = {
# #                             "Standard": fw_standard,
# #                             "Description": cfg.get("desc", in_desc),
# #                             "ReferenceCode": in_ref_code,
# #                         }
...
# #                         _set_if_present(cell_row, schema_cols, cfg.get("formula", ""), "CellFormula")
...
# #             st.success("Successfully added KPI rows.")
# #             st.cache_data.clear()
...
# #                             parent_table_rows = latest_master[
# #                                 latest_master[parent_code_col].astype(str) == str(parent_code_val)
# #                             ]
# #                             has_parent_table = parent_table_rows["KPIDetail"].apply(_is_parent_table_row).any()
# #                             if has_parent_table:
# #                                 codes_to_remove.update(
# #                                     parent_table_rows["IrisKPICode"].dropna().astype(str).tolist()
# #                                 )
...
# #                     st.success(
# #                         f"Removed {removed_count} row(s) for KPI selection: {selected_kpi_code}."
# #                     )
# #                     st.cache_data.clear()
# #                     st.rerun()
# #             except PermissionError:
...
# # st.subheader("🔵 Pillar Filters")
# # st.session_state.active_pillar = st.selectbox(
# #     "Pillar",
# #     options=["All", "Environmental", "Social", "Governance", "General"],
# #     index=["All", "Environmental", "Social", "Governance", "General"].index(st.session_state.active_pillar)
# #     if st.session_state.active_pillar in ["All", "Environmental", "Social", "Governance", "General"]
# #     else 0,
# # )
...
# #     "Select framework KPI to display",
# #     options=framework_options,
# #     index=framework_options.index(st.session_state.selected_framework)
# #     if st.session_state.selected_framework in framework_options
# #     else framework_options.index("GRI"),
# # )
...
# # if st.session_state.active_pillar != "All":
# #     biz_df = biz_df[biz_df["Pillar"] == st.session_state.active_pillar]
# # if search:
...
# #         "Filter Pillar",
# #         options=sorted([v for v in biz_df["Pillar"].dropna().astype(str).unique() if v.strip()]),
# #         placeholder="All Pillars",
# #     )
# # with f2:
# #     type_filter = st.multiselect(
# #         "Filter Type",
# #         options=sorted([v for v in biz_df["Type"].dropna().astype(str).unique() if v.strip()]),
# #         placeholder="All Types",
# #     )
# # with f3:
# #     agg_filter = st.multiselect(
# #         "Filter Aggregation",
# #         options=sorted([v for v in biz_df["Aggregation"].dropna().astype(str).unique() if v.strip()]),
# #         placeholder="All Aggregations",
# #     )

# # if pillar_filter:
# #     biz_df = biz_df[biz_df["Pillar"].isin(pillar_filter)]
# # if type_filter:
# #     biz_df = biz_df[biz_df["Type"].isin(type_filter)]
# # if agg_filter:
# #     biz_df = biz_df[biz_df["Aggregation"].isin(agg_filter)]

# # biz_df["Title (Master KPI)"] = biz_df["Title"]

# # m_cols = ["IrisKPICode", "Pillar", "Topic", "Type", "Title (Master KPI)", "Aggregation"]
# # selected_fw_col = f"{st.session_state.selected_framework} Language"
# # selected_fw_ref_col = f"{st.session_state.selected_framework} ReferenceCode"
...
# # gb.configure_column("Title (Master KPI)", editable=True)
...
# #             if not idx.empty:
# #                 latest_master.loc[idx, "Title"] = row["Title (Master KPI)"]
# #                 latest_master.loc[idx, "Category"] = inv_p.get(row["Pillar"], 1.0)
# #                 latest_master.loc[idx, "TopicId"] = inv_topic.get(row["Topic"], "T_01")
...
#     if pending_delete:
#         @st.dialog("⚠️ Confirm KPI Deletion")
#         def confirm_kpi_delete_dialog():
        ...
        if st.session_state.editing_cell is not None:
            ...
            def edit_cell_dialog(row_i, col_j):
                ...
                cagg_formula = st.text_input(
                    ...
                    key=f"dlg_agg_formula_{row_i}_{col_j}",
                )
                if not is_dynamic_table:
                    st.caption(f"Iris KPI Code: {preview_cell_codes.get((row_i, col_j), '—')}")
                if st.button("Save Cell", key=f"dlg_save_{row_i}_{col_j}"):
                    st.session_state.tabular_cell_data[(row_i, col_j)] = {
                        "kpi_type": ctype,
                        "title": ctitle,
