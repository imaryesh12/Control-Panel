...
# m_cols = ["IrisKPICode", "Pillar", "Topic", "Type", "Title (Master KPI)", "Aggregation"]
# if selected_framework_values:
#     selected_fw_lang_cols = [f"{fw_name} Language" for fw_name in selected_framework_values]
#     selected_fw_ref_cols = [f"{fw_name} ReferenceCode" for fw_name in selected_framework_values]
#     linked_mask = pd.Series(False, index=biz_df.index)
#     for col_name in selected_fw_lang_cols:
#         linked_mask = linked_mask | (~biz_df[col_name].astype(str).str.strip().isin(["", "—", "nan", "None"]))
#     biz_df = biz_df[linked_mask]
#     final_cols = m_cols + [val for pair in zip(selected_fw_lang_cols, selected_fw_ref_cols) for val in pair]
# else:
#     final_cols = m_cols
...
def render_dma_questionnaire_module():
    ...
    with st.expander("🟢 ADD NEW QUESTIONNAIRE"):
        ...
        with c1:
            ...
            in_industry = st.selectbox("Sub Industry", options=industry_options, key="dma_industry_input")
        with c2:
            context_type_options = sorted(
                [v for v in questionnaire_df[context_type_col].dropna().astype(str).unique() if v.strip()]
            )
    ...
    filtered_df = filtered_df.reset_index().rename(columns={"index": "_row_id"})
    preview_cols = ["_row_id", question_col, industry_col, context_type_col, updated_on_col]
    filtered_df = filtered_df[[col for col in preview_cols if col in filtered_df.columns]]
